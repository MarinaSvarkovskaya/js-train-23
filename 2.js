...
class Groups {
  ...
  create(name) {
    if (!this.#groups[name]) {
      this.#groups[name] = new Groups(name);
...
const electronics = Groups.create("Електроніка");
const books = Groups.create("Книги");
const electronics2 = Groups.create("Електроніка");
